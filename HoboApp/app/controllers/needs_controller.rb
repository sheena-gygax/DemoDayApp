class NeedsController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @need.save
        # longhand way to write do - end statement
        format.html { 
          flash[:notice] = 'Great news! Your listing has been added'
          redirect_to needs_url
        }
        format.json { render json: @need, status: :created, location: @need }
      else
        format.html { render action: "new", notice: 'Please complete fields' }
  ...
  def update
    ...
    respond_to do |format|
        if @need.update_attributes(params[:need])
          format.html { redirect_to needs_path, notice: 'Bam! Done' }
